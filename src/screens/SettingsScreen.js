  ...
  return (
    <SafeAreaView style={s.app}>
      <ScrollView contentContainerStyle={g.scroll}>
        ...
        <View style={g.group}>
          <SetRow ic="~" iconColor={C.gold} t="Weather & delay watch" sub="Predict disruptions before the airline" right={sw(weather, setWeather)} />
          <SetRow ic="v" iconColor={C.gold} t="Price & seat-drop alerts" sub="Better seats or fares on booked trips" right={sw(drops, setDrops)} />
          <View style={{ borderBottomWidth: 0 }}>
            <SetRow ic=")" iconColor={C.gold} t="Quiet hours" sub="Hold non-urgent pings 10p–7a" right={sw(quiet, setQuiet)} />
          </View>
        </View>

        <Text style={g.sectionT}>CONNECTED ACCOUNTS</Text>
        <View style={g.group}>
          <SetRow ic="@" iconColor={C.gold} t="Inbox" sub="maddie@…  · syncing" right={<Chip color={C.teal}>Connected</Chip>} />
          <SetRow ic="$" iconColor={C.gold} t="Payment & trip protection" sub="Visa ••42 · covered rebookings" right={<Chip>Manage</Chip>} />
...
const s = StyleSheet.create({
  ...
  cap:  { color: C.mut, fontSize: 13, fontFamily: T.sans, marginTop: 12, lineHeight: 20 },
